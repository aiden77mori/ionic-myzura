...
@Injectable()
export class ProductsFilterProvider {

  private _filterProducts$ = new BehaviorSubject<any[]>([]);
  batch = 15;
  startKey = 0;
  finished = false;
  userId
  ...
  // }
  get filterProducts$(): Observable<any[]> {
    return this._filterProducts$.asObservable();
  }
  destroy(){
    this._filterProducts$ = new BehaviorSubject<any[]>([]);
    this.batch = 15;
    this.startKey = 0;
    this.finished = false;
  }
  nextPage(): Observable<any[]> {
    if (this.finished) { return this.filterProducts$; }
        ...
        tap(productsIds => {
          ...
          if(productsIds.length && this.startKey<this.productsListIDs.length){ 
            this.getProductsByIds(productsIds).then(products =>{
              ...
              const currentProducts = this._filterProducts$.getValue();
              ...
              this._filterProducts$.next(currentProducts.concat(newProducts));
