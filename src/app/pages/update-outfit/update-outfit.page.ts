...
@Component({
  selector: 'app-update-outfit',
  templateUrl: './update-outfit.page.html',
  styleUrls: ['update-outfit.page.scss']

})
export class UpdateOutfitPage implements OnInit {
 ...
 //   ,  { type: 'pattern', message: 'Enter a valid email.' }
  ...
  constructor(public menu: MenuController,
   ...
   ionViewWillEnter(){
    ...
    this.arouter.queryParams.subscribe(params => {
     console.log(params)
     if (params && params.outfitId) {
       // alert(1)
       this.outfitId=params.outfitId
      ...
      if(!this.outfitId){
        this.arouter.queryParams.subscribe(params => {
          ...
          if (params && params.outfitId) {
            // alert(1)
            this.outfitId=params.outfitId
            this.dataProvider.angularDb.object("outfits/"+ this.userId+ "/" + this.outfitId).query.once("value",(res:any)=>{
              res = res.val()
              console.log(res)
              this.draggables = JSON.parse(res.outfit)
              this.outfitForm.patchValue({"name":res.name })
            })
          }
        });
      }else{
  ...
  async save(){  
    ...
    loading.present();  
    this.dataProvider.angularDb.object("outfits/"+ this.userId+"/"+this.outfitId).update({
